...
import { IncomeRowData, IncomeEntryRequest } from '@/types/income'
...
export default function IncomePage() {
  ...
  const initializeRows = (branchList: Branch[]) => {
    const initialRows: IncomeRowData[] = branchList.map((branch) => ({
      branch_id: branch.id,
  ...
  const handleInputChange = (index: number, field: keyof IncomeRowData, value: string) => {
    setRows((prev) => {
      const updated = [...prev]
      ...
      if (field === 'omzet' || field === 'pemasukan_toru') {
        row[field] = displayValue
        const omzet = parseNumber(row.omzet)
        const toruInput = parseNumber(row.pemasukan_toru)
        row.pemasukan_cash = omzet - toruInput
      } else if (
        field === 'qris_bca' ||
        field === 'qris_bni' ||
        field === 'qris_bri' ||
      ...
      const delta = row.pemasukan_cash - totalPayments
      row.difference = delta

      if (delta < 0) {
        const over = Math.abs(delta)
        row.status = 'over'
        row.warning = `Cabang ${row.branch_name}: kelebihan ${formatCurrency(over)} (bayar ${formatCurrency(totalPayments)} > cash ${formatCurrency(row.pemasukan_cash)})`
      } else if (delta > 0 && row.pemasukan_cash > 0) {
        row.status = 'under'
        row.warning = `Cabang ${row.branch_name}: sisa ${formatCurrency(delta)} (cash ${formatCurrency(row.pemasukan_cash)} > bayar ${formatCurrency(totalPayments)})`
      } else {
  ...
  const confirmSave = async () => {
    ...
    try {
      const requests: IncomeEntryRequest[] = rows
        .filter((row) => parseNumber(row.omzet) > 0)
        .map((row) => ({
          branch_id: row.branch_id,
          date: selectedDate,
          omzet: parseNumber(row.omzet),
          pemasukan_toru: parseNumber(row.pemasukan_toru),
      ...
      const results = await Promise.allSettled(
        requests.map((req) => apiClient.post('/income-entries', req))
      )
      ...
      if (failed > 0) {
        ...
        toast({
          title: 'Berhasil',
          description: `${successful} data pemasukan berhasil disimpan`
        })
        initializeRows(branches)
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tabel Input Pemasukan per Cabang</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[120px]">Pemasukan Toru</TableHead>
                  <TableHead className="w-[120px] bg-muted/50">Pemasukan Cash</TableHead>
                ...
                {rows.map((row, index) => (
                  <TableRow key={row.branch_id} className={row.warning ? 'bg-red-50' : ''}>
                    ...
                    <TableCell>
                      <Input
                        type="text"
                        value={row.pemasukan_toru}
                        onChange={(e) => handleInputChange(index, 'pemasukan_toru', e.target.value)}
                        placeholder="0"
                        className="w-full"
                      />
                    </TableCell>
                    <TableCell className="bg-muted/50">
                      <div className="font-medium text-sm">
                        {formatCurrency(row.pemasukan_cash)}
                      </div>
                    </TableCell>
                    <TableCell>
                      <Input
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Simpan data pemasukan untuk tanggal {selectedDate}?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
